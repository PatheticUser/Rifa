...
export const VideoChat: React.FC<VideoChatProps> = ({
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-rose-900/20 via-purple-900/30 to-pink-900/20 backdrop-blur-sm flex flex-col">
      ...
      {/* Video Area */}
      <div className="flex-1 relative p-2 md:p-4 flex justify-center items-center">
        <div className="relative w-full max-w-7xl aspect-video bg-black/20 rounded-xl overflow-hidden backdrop-blur-sm border border-white/10">
          {/* Remote Video */}
          {remoteStream ? (
            <video
              ref={remoteVideoRef}
            ...
            <div className="w-full h-full flex items-center justify-center">
              <div className="text-center">
                ...
                <p className="text-white/60">
                  Share room code:{" "}
                  <span className="font-mono text-pink-300 bg-white/10 px-2 py-1 rounded">
                    {roomId}
                  </span>
                </p>
                {!isSignalingConnected && (
                  <p className="text-red-300 text-sm mt-2">
                    ⚠️ Server connection lost. Please refresh the page.
                  </p>
                )}
              </div>
            </div>
          ...
          {/* Local Video (PIP) */}
          <div className="absolute bottom-4 right-4 w-32 h-24 md:w-48 md:h-36 bg-black/30 rounded-xl overflow-hidden border border-white/20 backdrop-blur-sm">
            {localStream ? (
              <video
                ref={localVideoRef}
                autoPlay
      ...
      {/* Controls */}
      <div className="bg-black/30 backdrop-blur-xl border-t border-white/10 p-4 md:p-6">
